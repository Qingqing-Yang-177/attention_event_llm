...
check_punct=int(sys.argv[2]) # a binary whether we are checking punctuation

# Setup path to save the results to npy files
foldername = os.path.splitext(text_tested)[0] # Extract the filename without the extension
output_dir = f'./attention_event/results/{foldername}_noseg'
os.makedirs(output_dir, exist_ok=True)

# Setup the input file path
story_dir = './attention_event/story'
...
# define prompt for noseg
# this is the main difference between two python script.
prompt= f''' Copy the following story word-for-word.

This is the story: {story}

This is a word-for-word copy of the same story:'''

# setup prompt and tokenization, which is also different compared to the seg condition
messages = [
    {"role": "system", "content": "You are a person listening to a continuous story. "},
